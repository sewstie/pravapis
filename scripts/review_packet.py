...
def main(argv: list[str]) -> int:
    ...
    lines.append(
        ...
        "- **Ва Ўкраіне or ва Украіне?** After a word ending in a vowel, does ў replace "
        "у at the start of a **capitalised proper noun** — Ўкраіна, Ўсходняя, Ўроцлаў, "
        "Ўілсан? be-tarask writes ў in all nine corpus cases; be.wikipedia writes у. The "
        "rule would be §15, which could not be read (the book is not redistributable and "
        "is absent from the checkout), so nothing is implemented. This is the largest "
        "rule-shaped gap the recall harness has found — the `other` class scores 0 of 35 "
        "and thirty of those are this question.\n"
