...
def test_core_import_does_not_load_disambiguate() -> None:
    code = (
        "import sys, belnorm, belnorm.pipeline, belnorm.api.main; "
        "belnorm.convert('план', belnorm.Orthography.TARASKIEVICA); "
        "print(any(m.startswith('belnorm.disambiguate') for m in sys.modules))"
    )
